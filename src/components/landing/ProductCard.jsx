...
export default function ProductCard({ product, index, dark = false }) {
        ...
        >
            <Link to={`/produto/${product.id}`} className="block">
                <div className="relative bg-gray-50 rounded-xl sm:rounded-2xl overflow-hidden aspect-square mb-3 sm:mb-4">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-lime-500 hover:text-slate-900 text-white py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm transition-all duration-300"
                        >
                ...
                <div className="px-0.5 sm:px-1">
                    <p className={`text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider mb-0.5 sm:mb-1 ${dark ? "text-slate-400" : "text-slate-400"}`}>
                        {product.category}
                    </p>
                    <h3 className={`font-semibold text-xs sm:text-sm leading-snug mb-1.5 sm:mb-2 group-hover:text-lime-500 transition-colors line-clamp-2 ${dark ? "text-white" : "text-slate-900"}`}>
                        {product.name}
                    </h3>
                    <div className="flex items-baseline gap-1.5 sm:gap-2 flex-wrap">
                        <span className={`text-base sm:text-lg font-bold ${dark ? "text-white" : "text-slate-900"}`}>
                            {product.price}
                        </span>
                        {product.originalPrice && (
                            <span className={`text-xs sm:text-sm line-through ${dark ? "text-slate-500" : "text-slate-400"}`}>
                                {product.originalPrice}
                            </span>
                        )}
                    </div>
